...
export function MultiPhotoScanScreen() {
  ...
  const renderSatellitePhoto = (photoIndex: number) => (
    <View style={[styles.satellitePhoto, { borderColor: photos[photoIndex] ? colors.success : colors.border }]}> 
      {photos[photoIndex] ? (
        <>
          <Image source={{ uri: photos[photoIndex].uri }} style={styles.satellitePhotoImage} />
          ...
          <View style={styles.scanningContainer}>
            ...
            <View style={styles.progressSection}>
              <View style={styles.statusCheckmark}>
                ...
                <Text style={[styles.statusText, { color: colors.text }]}>{scanMessage}</Text>
              </View>
              ...
              <Text style={[styles.progressText, { color: colors.textSecondary }]}>{scanProgress}%</Text>
            </View>
          </View>
        </View>
      </SafeAreaView>
      ...
      <View style={{ flex: 1, backgroundColor: colors.background }}>
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: Spacing.xl }} showsVerticalScrollIndicator={false}>
          <View style={{ paddingHorizontal: Spacing.lg, paddingTop: Spacing.lg, paddingBottom: Spacing.md }}>
            <Text style={{ fontSize: fontSizes['2xl'], fontWeight: FontWeights.bold, color: colors.text, marginBottom: Spacing.sm }}>Multi-Scan</Text>
            <Text style={{ fontSize: fontSizes.sm, color: colors.textSecondary, lineHeight: 20 }}>Prenez 3 photos de votre peau sous différents angles pour une analyse précise</Text>
          </View>
          ...
          {!loadingUsage && usage && (
            <Card style={{ marginHorizontal: Spacing.lg, marginBottom: Spacing.lg }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: Spacing.md }}>
                ...
                <Text style={{ fontSize: fontSizes.sm, fontWeight: FontWeights.medium, color: colors.text }}>Quota d'analyses</Text>
              </View>
              <Text style={{ fontSize: fontSizes.sm, color: colors.textSecondary }}>
                {usage.quotas.analyses.remaining !== null ? `${usage.quotas.analyses.remaining} / ${usage.quotas.analyses.limit} analyses restantes` : 'Analyses illimitées (Premium)'}
              </Text>
            </Card>
          )}

          <View style={{ paddingHorizontal: Spacing.lg, marginBottom: Spacing.lg }}>
            <ImagePicker onImageSelected={handleImageSelected} label="Sélectionner une photo" showPreview={true} aspectRatio={[4, 5]} />
          </View>

          <View style={{ paddingHorizontal: Spacing.lg, marginBottom: Spacing.lg }}>
            <Text style={{ fontSize: fontSizes.sm, fontWeight: FontWeights.semibold, color: colors.text, marginBottom: Spacing.md }}>Photos capturées ({photos.filter((photo): photo is CapturedPhoto => Boolean(photo?.base64)).length}/{REQUIRED_PHOTOS})</Text>
            <View style={styles.photoSlotsContainer}>
              {Array(REQUIRED_PHOTOS).fill(null).map((_, index) => (
                <View key={index} style={styles.photoSlot}>
                  {photos[index] ? (
                    <>
                      <Image source={{ uri: photos[index].uri }} style={styles.photoSlotImage} />
                      <View style={styles.photoOverlay}>
                        <TouchableOpacity style={[styles.removeButton, { backgroundColor: colors.warning }]} onPress={() => removePhoto(index)}>
                    ...
                    <TouchableOpacity style={[styles.photoSlotEmpty, { borderColor: colors.border }]} onPress={() => addPhoto(index)}>
                      <Ionicons name="camera-outline" size={32} color={colors.primary} />
                      <Text style={[styles.photoSlotEmptyText, { color: colors.textSecondary }]}>Photo {index + 1}</Text>
                      {selectedImage && <Text style={[styles.tapText, { color: colors.primary }]}>Appuyez pour ajouter</Text>}
                    </TouchableOpacity>
                  )}
                </View>
              ))}
...
const styles = StyleSheet.create({
  ...
  statusCheckmark: { flexDirection: 'row', alignItems: 'center', marginBottom: Spacing.md, gap: Spacing.sm },
  statusText: { fontSize: FontSizes.base, fontWeight: FontWeights.medium, flex: 1 },
  progressBar: { height: 8, borderRadius: 4, overflow: 'hidden', marginBottom: Spacing.sm },
  progressFill: { height: '100%', borderRadius: 4 },
  progressText: { fontSize: FontSizes.xs, textAlign: 'center' },
  photoSlotsContainer: { gap: Spacing.md },
  photoSlot: { height: 140, borderRadius: BorderRadius.lg, overflow: 'hidden', position: 'relative', ...Shadows.md },
  photoSlotImage: { width: '100%', height: '100%' },
  photoOverlay: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.3)', justifyContent: 'flex-end', alignItems: 'flex-end', padding: Spacing.md },
  removeButton: { width: 36, height: 36, borderRadius: 18, justifyContent: 'center', alignItems: 'center' },
  photoCheckmark: { position: 'absolute', top: Spacing.md, right: Spacing.md },
  photoSlotEmpty: { flex: 1, borderWidth: 2, borderStyle: 'dashed', borderRadius: BorderRadius.lg, justifyContent: 'center', alignItems: 'center', gap: Spacing.sm },
  photoSlotEmptyText: { fontSize: FontSizes.sm, fontWeight: FontWeights.medium },
  tapText: { fontSize: FontSizes.xs, marginTop: Spacing.xs },
});
